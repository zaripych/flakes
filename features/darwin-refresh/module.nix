  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.systemPackages = [
      (
        let
          git-add =
            if builtins.length cfg.gitAddPaths > 0
            then (builtins.concatStringsSep "\n" (builtins.map (path: "git -C ${path} add .") cfg.gitAddPaths))
            else "";
          refresh-inputs =
            if builtins.length cfg.updateInputs > 0
            then "nix flake update --flake ${cfg.flakePath} ${builtins.concatStringsSep " " cfg.updateInputs}"
            else "";
        in (pkgs.writeShellScriptBin "darwin-refresh" ''
          ${git-add}
          ${refresh-inputs}
          sudo darwin-rebuild --flake ${cfg.flakePath} $@
        '')
      )
    ];
  };
