{config, ...}: let
  ...
  traceList = prefix: list: elementPrinter:
    builtins.trace
  ...
  syncedApps =
    traceList "Using nix-darwin to install (environment.syncedApps):" (
      if config.environment ? syncedApps
      then config.environment.syncedApps
      else []
    )
    packageInfo;
  userPackages =
    traceList "Using home-manager to install (environment.syncedApps):" (
      if config ? home-manager
      then config.home-manager.users.${username}.home.packages
      else []
    )
    packageInfo;
in {
